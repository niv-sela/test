...
2. Asset ID Retrieval (XQL): Run XQL query process to get all the relevant assets based on their relations. 
Obtain all relevant asset IDs using the TextToXQL tool, by querying the asset_inventory dataset using XQL. The query will look like this:
...
* If new asset IDs are discovered through XQL, re-iterate and run XQL again to find more asset ids
